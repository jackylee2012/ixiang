...
BarChart.prototype = {
    ...
    drawAxis: function() {
        ...
        if(this.unit && !!this.unit.length){
            var xUnit = this.unit[0],
                yUnit = this.unit[1] || '';
            this.ctx.beginPath();
            this.ctx.fillStyle = this.titleColor;
            this.ctx.textAlign="start";
            this.ctx.font = '16px Microsoft YaHei';
            this.ctx.fillText(xUnit,this.width - this.padding / 2 + 5, this.height - this.padding + 5);
            this.ctx.textAlign="center";
            !!yUnit && this.ctx.fillText(yUnit,this.padding + 0.5, this.padding -5 )
        }
    },
    drawPoint: function() {
        // x轴坐标点
    ...
    drawUnit:function () {
        if(this.unit && !!this.unit.length){
            ...
            this.ctx.fillStyle = this.titleColor;
            this.ctx.font = '16px Microsoft YaHei';
            this.ctx.fillText(xUnit,this.width - this.padding/2,this.height-this.padding/2 - 2);
            !!yUnit && this.ctx.fillText(yUnit,this.padding + 0.5, this.padding + 0.5 -2)
        }
    },
    /**
     * 监听鼠标移动事件
    ...
    getYFictitious: function(data) {
        ...
        pow = pow > 2 ? 2 : pow;
        console.log(Math.ceil(len / Math.pow(10,pow)) * Math.pow(10,pow))
